...
def get_random_item(items : list, probabilities : list=None):
	# No probabilities given, so make it evenly distributed
	if probabilities == None:
		probabilities = get_default_probabilities(items=items)
...
def get_default_probabilities(items : list) -> list:
